...
@section('content')
    ...
    <div class="container" style="margin-top: 20px;">
        <h3 class="text-center"><b style="color: rgb(8, 7, 20)">User Transactions</b></h3>
        ...
        <div class="table-responsive">
            <table class="table table-striped" id="utrans-tbl">
                <thead>
                    <tr>
                        <th>Sl.No</th>
                        <th>Name</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            function transactionsLoad(page = 1, formElement) {
                ...
                $.ajax({
                    url: "{{ route('admin.all.users.transactions') }}",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                            ...
                            response.data.data.forEach(function(item) {
                                ...
                                rows += `
                                        <tr>
                                            ...
                                            <td>${item.amount_wallet ?? ''}</td>
                                            <td>${item.reward_amount ?? ''}</td>
                                            <td>${item.amount ?? ''}</td>
                                            <td>₹${item.remaining_amount ?? 0}/-</td>
                                            <td>₹${item.remaining_reward ?? 0}/-</td>
                                            <td>${item.trans_date ?? 0}</td>
                                        </tr>
                                    `;
                            });
            ...
            // Filter submit
            $('#trans-form').on('submit', function(e) {
                e.preventDefault();
                transactionsLoad(1, this);
            });

            // Pagination click
            $(document).on('click', '.pagination-btn', function() {
                const page = $(this).data('page');
                transactionsLoad(page, $('#srch-form')[0]); // Reuse the same form
            });
